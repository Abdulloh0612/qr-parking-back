...
Если QR **уже зарегистрирован**: при **совпадении** `phone` с владельцем — токен владельца; если номер **другой**, но в системе **есть аккаунт** с этим телефоном — выдаётся токен **этого** пользователя; если номера нет в базе — ошибка валидации.
